...
class OfflineActiveVisionEnv(gym.Env):
    ...
    def __init__(self, data_dir: str):
        ...
        self.observation_space  = spaces.Dict(
            {
                "img": spaces.Box(low=0, high=1, shape=dummy_img.shape, dtype=dummy_img.dtype),
                "cam2world_matrix": spaces.Box(low=-20, high=20, shape=dummy_cam2world_matrix.shape, dtype=dummy_cam2world_matrix.dtype)
            }
        )
        
        self.action_space = spaces.Discrete(len(self.frames))

        self._action_to_cam2world_matrix = {i: frame['transform_matrix'] for i, frame in enumerate(self.frames)}
